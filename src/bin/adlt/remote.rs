...
fn fs_cmd_archive(
    ...
    if uri.len() != 2 && !full_path.ends_with('!') {
        return Err(std::io::Error::new(
            std::io::ErrorKind::InvalidInput,
            format!(
                "path '{full_path}' not in expected format 'path/to.archive!/path/within/archive'"
            ),
        )
        .into());
    }
    let (archive_path, path_within) = match uri.len() {
        ...
        _ => {
            return Err(std::io::Error::new(
                std::io::ErrorKind::InvalidInput,
                format!("path '{full_path}' not in expected format"),
            )
            .into())
        }
    };
...
fn binary_search_by_msg_index(
    ...
    if fc.sort_by_time {
        ...
        if let Some((all_msgs_idx, msg)) = wanted_msg {
            ...
            Err(format!("sort_by_time, index={wanted_msg_idx} unknown"))
        }
    } else {
        // sort by index
        ...
        if let Ok(all_msgs_idx) = all_msgs_idx {
            ...
            Err(format!(
                "sort_by_index, index={wanted_msg_idx} not in all_msgs"
            ))
        }
    }
}
